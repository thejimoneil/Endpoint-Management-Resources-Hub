# Endpoint Management Resources Hub

A comprehensive collection of educational, informational, and networking resources for Endpoint Management professionals. This includes resources for Microsoft Intune, Configuration Manager (ConfigMgr/SCCM), PowerShell, and related technologies.

> **Community Contributions Welcome!** Feel free to submit pull requests to add valuable resources, update links, or improve organization.

---

## Table of Contents

- [Microsoft Intune](#microsoft-intune)
  - [Official Documentation](#official-documentation)
  - [Learning Paths & Certifications](#learning-paths--certifications)
  - [Blogs & Community Sites](#blogs--community-sites)
  - [Tools & Utilities](#tools--utilities)
- [Configuration Manager (SCCM/ConfigMgr)](#configuration-manager-sccmconfigmgr)
  - [Official Resources](#official-resources)
  - [Community Resources](#community-resources)
  - [Tools & Scripts](#tools--scripts)
- [PowerShell](#powershell)
  - [Learning Resources](#learning-resources)
  - [PowerShell for Intune](#powershell-for-intune)
  - [PowerShell for ConfigMgr](#powershell-for-configmgr)
  - [Best Practices](#best-practices)
- [Community & Networking](#community--networking)
  - [Forums & Discussion Groups](#forums--discussion-groups)
  - [Social Media](#social-media)
  - [User Groups](#user-groups)
  - [Conferences](#conferences)
  - [Media & Podcasts](#media--podcasts)
  - [Webinars](#webinars)
  - [Community Tools](#community-tools)
- [Security & Compliance](#security--compliance)
- [Troubleshooting & Support](#troubleshooting--support)
- [Additional Tools](#additional-tools)
- [Contributing](#contributing)

---

## Microsoft Intune

### Official Documentation

- **[Microsoft Intune Documentation](https://learn.microsoft.com/en-us/mem/intune/)** - Official Microsoft Learn documentation for Intune
- **[What's New in Intune](https://learn.microsoft.com/en-us/mem/intune/fundamentals/whats-new)** - Monthly updates on new features and changes
- **[Intune Planning Guide](https://learn.microsoft.com/en-us/mem/intune/fundamentals/intune-planning-guide)** - Comprehensive planning and deployment guide
- **[Intune App Management](https://learn.microsoft.com/en-us/mem/intune/apps/)** - Documentation for application deployment and management
- **[Microsoft Endpoint Manager Admin Center](https://endpoint.microsoft.com/)** - The admin portal for Intune

### Learning Paths & Certifications

- **[Microsoft Certified: Endpoint Administrator Associate](https://learn.microsoft.com/en-us/certifications/modern-desktop/)** - Official certification for endpoint administrators
- **[Microsoft Learn: Deploy and Manage Windows 365](https://learn.microsoft.com/en-us/training/paths/deploy-manage-windows-365/)** - Learning path for Windows 365
- **[Intune Training on Microsoft Learn](https://learn.microsoft.com/en-us/training/browse/?products=mem-intune)** - Free training modules
- **[MS-102: Microsoft 365 Administrator](https://learn.microsoft.com/en-us/certifications/exams/ms-102)** - Includes Intune administration

### Blogs & Community Sites

- **[Intune Customer Success Blog](https://techcommunity.microsoft.com/t5/intune-customer-success/bg-p/IntuneCustomerSuccess)** - Official Microsoft Tech Community blog
- **[MSEndpointMgr.com](https://msendpointmgr.com/)** - Comprehensive blog by Microsoft MVPs (formerly SCConfigMgr)
- **[Oliver Kieselbach's Blog](https://oliverkieselbach.com/)** - Deep technical insights on Intune
- **[Peter van der Woude's Blog](https://www.petervanderwoude.nl/)** - Extensive coverage of Intune and ConfigMgr
- **[Call4Cloud.nl](https://call4cloud.nl/)** - Creative and detailed Intune guides by Rudy Ooms
- **[Andrew Taylor's Blog](https://andrewstaylor.com/)** - Automation and scripting for Intune
- **[Petri.com Intune Section](https://petri.com/category/microsoft-intune/)** - News and how-to guides
- **[Insentra Blog](https://insentragroup.com/au/insights/)** - Enterprise deployment insights

### Tools & Utilities

- **[Microsoft Intune App Wrapping Tool](https://learn.microsoft.com/en-us/mem/intune/developer/apps-prepare-mobile-application-management)** - Wrap apps for MAM
- **[Intune Management Extension Log Analyzer](https://github.com/MSEndpointMgr/IntuneManagementExtension)** - Analyze IME logs
- **[IntuneWin App Utility](https://github.com/microsoft/Microsoft-Win32-Content-Prep-Tool)** - Package Win32 apps for Intune
- **[PSIntuneAuth](https://github.com/MSEndpointMgr/MSGraphRequest)** - PowerShell module for Intune authentication
- **[Intune PowerShell SDK](https://github.com/microsoftgraph/Intune-PowerShell-SDK)** - Official PowerShell SDK
- **[IntuneBackupAndRestore](https://github.com/jseerden/IntuneBackupAndRestore)** - Backup and restore Intune configurations

---

## Configuration Manager (SCCM/ConfigMgr)

### Official Resources

- **[Configuration Manager Documentation](https://learn.microsoft.com/en-us/mem/configmgr/)** - Official Microsoft documentation
- **[What's New in ConfigMgr](https://learn.microsoft.com/en-us/mem/configmgr/core/plan-design/changes/whats-new-incremental-versions)** - Updates and new features
- **[ConfigMgr Release Notes](https://learn.microsoft.com/en-us/mem/configmgr/core/servers/deploy/install/release-notes)** - Known issues and changes
- **[ConfigMgr Support Center](https://learn.microsoft.com/en-us/mem/configmgr/core/support/support-center)** - Troubleshooting tool

### Community Resources

- **[System Center Dudes](https://www.systemcenterdudes.com/)** - Community blog with extensive guides
- **[Anoop C Nair's Blog](https://www.anoopcnair.com/)** - Microsoft MVP with detailed ConfigMgr content
- **[ConfigMgr Reddit Community](https://www.reddit.com/r/SCCM/)** - Active community forum
- **[Niall Brady's Blog (Windows Noob)](https://www.windows-noob.com/)** - Step-by-step tutorials
- **[Prajwal Desai's Blog](https://www.prajwaldesai.com/)** - ConfigMgr and Intune guides
- **[GARYTOWN ConfigMgr Blog](https://garytown.com/)** - Scripts, guides, and community content

### Tools & Scripts

- **[ConfigMgr Client Center](https://github.com/rzander/sccmclictr)** - Client management and troubleshooting tool
- **[CMPivot](https://learn.microsoft.com/en-us/mem/configmgr/core/servers/manage/cmpivot)** - Real-time query tool
- **[Configuration Manager PowerShell Cmdlets](https://learn.microsoft.com/en-us/powershell/sccm/overview)** - Official cmdlets
- **[OSD Toolkit](https://www.osdeploy.com/)** - OSD Front End by David Segura
- **[Modern Driver Management](https://msendpointmgr.com/modern-driver-management/)** - Automated driver management solution

---

## PowerShell

### Learning Resources

- **[PowerShell Documentation](https://learn.microsoft.com/en-us/powershell/)** - Official Microsoft PowerShell docs
- **[PowerShell Gallery](https://www.powershellgallery.com/)** - Repository for PowerShell modules
- **[Learn PowerShell in a Month of Lunches](https://www.manning.com/books/learn-powershell-in-a-month-of-lunches)** - Popular book by Don Jones
- **[PowerShell.org](https://powershell.org/)** - Community resources and forums
- **[PowerShell Explained](https://powershellexplained.com/)** - Comprehensive guides by Kevin Marquette
- **[AdamTheAutomator](https://adamtheautomator.com/powershell/)** - PowerShell tutorials and guides

### PowerShell for Intune

- **[Microsoft Graph PowerShell SDK](https://learn.microsoft.com/en-us/powershell/microsoftgraph/overview)** - Interact with Microsoft Graph
- **[Intune Graph API Samples](https://github.com/microsoftgraph/powershell-intune-samples)** - Example scripts for Intune automation
- **[PSIntuneAuth Module](https://www.powershellgallery.com/packages/PSIntuneAuth/)** - Simplify Intune authentication
- **[IntuneDocumentation Module](https://github.com/ThomasKur/IntuneDocumentation)** - Document your Intune configuration

### PowerShell for ConfigMgr

- **[ConfigMgr PowerShell Cmdlets Library](https://learn.microsoft.com/en-us/powershell/sccm/overview)** - Complete cmdlet reference
- **[SCCM PowerShell Scripts Collection](https://github.com/topics/sccm-powershell)** - Community scripts on GitHub
- **[ConfigMgr Client SDK](https://learn.microsoft.com/en-us/previous-versions/system-center/developer/cc143010(v=msdn.10))** - WMI classes for automation

### Best Practices

- **[PowerShell Best Practices and Style Guide](https://poshcode.gitbook.io/powershell-practice-and-style/)** - Community style guide
- **[The PowerShell Best Practices and Style Guide](https://github.com/PoshCode/PowerShellPracticeAndStyle)** - GitHub repository
- **[PowerShell Security Best Practices](https://learn.microsoft.com/en-us/powershell/scripting/security/powershell-security-best-practices)** - Official security guidance

---

## Community & Networking

### Forums & Discussion Groups

- **[Microsoft Tech Community - Intune](https://techcommunity.microsoft.com/t5/microsoft-intune/bd-p/Microsoft-Intune)** - Official Microsoft forum
- **[Reddit /r/Intune](https://www.reddit.com/r/Intune/)** - Active Reddit community
- **[Reddit /r/SCCM](https://www.reddit.com/r/SCCM/)** - ConfigMgr community
- **[WinAdmins Discord](https://aka.ms/winadmins)** - Large Windows admin community with Intune channels
- **[PowerShell Discord](https://aka.ms/psdiscord)** - PowerShell community server

### Social Media

- **Twitter/X Lists:**
  - **#MEM** - Microsoft Endpoint Manager discussions
  - **#Intune** - Intune-specific content
  - **#PowerShell** - PowerShell community
  - **#SCCM** - Configuration Manager topics
  
- **Notable Twitter/X Accounts:**
  - [@IntuneSuppTeam](https://twitter.com/IntuneSuppTeam) - Official Intune support
  - [@SteveMustCSE](https://twitter.com/SteveMustCSE) - Microsoft PM for Intune
  - [@jannikreinhard](https://twitter.com/jannikreinhard) - Microsoft MVP
  - [@MrTechGuy_](https://twitter.com/MrTechGuy_) - Ben Reader, Intune MVP

### User Groups

- **[Microsoft Endpoint Manager User Group (MEMUG)](https://memug.com/)** - Virtual and in-person user groups worldwide
- **[Local Microsoft User Groups](https://www.meetup.com/topics/microsoft/)** - Find local meetups via Meetup.com
- **[PowerShell User Groups](https://www.meetup.com/topics/powershell/)** - PowerShell-focused community groups
- **[Cloud & Datacenter Management User Group](https://www.linkedin.com/groups/)** - Professional networking groups

### Conferences

- **[Microsoft Ignite](https://ignite.microsoft.com/)** - Annual flagship Microsoft conference
- **[Midwest Management Summit (MMS)](https://mmsmoa.com/)** - Premier ConfigMgr/Intune conference
- **[Microsoft Build](https://build.microsoft.com/)** - Annual developer conference with endpoint management content
- **[IT/Dev Connections](https://www.itdevconnections.com/)** - Multi-track IT conference
- **[BITSConf](https://www.bitsconf.com/)** - IT professionals conference

### Media & Podcasts

- **[Intune Training Videos](https://www.youtube.com/c/IntuneTraining)** - Dedicated Intune YouTube channel
- **[John Bryntze YouTube Channel](https://www.youtube.com/@JohnBryntze)** - Intune and M365 content
- **[Microsoft Mechanics](https://www.youtube.com/user/OfficeGarageSeries)** - Official Microsoft technical content
- **[MS Endpoint Manager Podcast](https://www.msendpointmgr.com/podcast/)** - Regular podcast on endpoint management
- **[RunAsRadio](https://www.runasradio.com/)** - IT pro podcast featuring endpoint management topics
- **[Intune.Training YouTube](https://www.youtube.com/@IntuneTraining)** - Training videos and walkthroughs
- **[MSEndpointMgr YouTube](https://www.youtube.com/@MSEndpointMgr)** - Community content and how-tos

### Webinars

- **[Microsoft Intune Webinars](https://aka.ms/IntuneWebinars)** - Official Microsoft Intune webinar series
- **[Microsoft Tech Community Events](https://techcommunity.microsoft.com/t5/events/ct-p/Events)** - Regular technical webinars and events
- **[WinAdmins Community Calls](https://winadmins.io/)** - Community-led webinars and discussions
- **[Intune Customer Success Webinars](https://techcommunity.microsoft.com/t5/intune-customer-success/bg-p/IntuneCustomerSuccess)** - Best practices and feature deep-dives

### Community Tools

- **[WinAdmins Tools Collection](https://winadmins.io/tools)** - Curated list of community tools
- **[MSEndpointMgr Tools](https://msendpointmgr.com/tools/)** - Free community tools and utilities
- **[Intune Toolkit by Andrew Taylor](https://andrewstaylor.com/tools/)** - Collection of automation scripts and tools
- **[PSAppDeployToolkit](https://psappdeploytoolkit.com/)** - PowerShell-based application deployment framework
- **[ModernWorkplace.Tools](https://modernworkplace.tools/)** - Collection of M365 and Intune utilities

---

## Security & Compliance

- **[Microsoft Defender for Endpoint](https://learn.microsoft.com/en-us/microsoft-365/security/defender-endpoint/)** - Endpoint security platform
- **[Microsoft Compliance Manager](https://learn.microsoft.com/en-us/microsoft-365/compliance/compliance-manager)** - Compliance assessment tool
- **[Intune Security Baselines](https://learn.microsoft.com/en-us/mem/intune/protect/security-baselines)** - Recommended security configurations
- **[Windows Security Baselines](https://learn.microsoft.com/en-us/windows/security/operating-system-security/device-management/windows-security-configuration-framework/windows-security-baselines)** - Microsoft security recommendations
- **[CIS Benchmarks](https://www.cisecurity.org/cis-benchmarks)** - Industry-standard security configurations
- **[NIST Cybersecurity Framework](https://www.nist.gov/cyberframework)** - Framework for managing cybersecurity risk

---

## Troubleshooting & Support

- **[Intune Support](https://learn.microsoft.com/en-us/mem/get-support)** - How to get Microsoft support
- **[Intune Troubleshooting Portal](https://endpoint.microsoft.com/#blade/Microsoft_Intune_DeviceSettings/TroubleshootBlade)** - Built-in troubleshooting
- **[ConfigMgr Support Center](https://learn.microsoft.com/en-us/mem/configmgr/core/support/support-center)** - Troubleshooting toolkit
- **[Log Locations for Intune](https://learn.microsoft.com/en-us/troubleshoot/mem/intune/device-protection/troubleshoot-policies-in-microsoft-intune#log-locations)** - Where to find diagnostic logs
- **[ConfigMgr Log Files](https://learn.microsoft.com/en-us/mem/configmgr/core/plan-design/hierarchy/log-files)** - Complete log file reference
- **[CMTrace Log Viewer](https://learn.microsoft.com/en-us/mem/configmgr/core/support/cmtrace)** - Essential log viewing tool

---

## Additional Tools

### General Endpoint Management

- **[Microsoft Endpoint Manager Admin Center](https://endpoint.microsoft.com/)** - Unified management portal
- **[Azure AD Portal](https://aad.portal.azure.com/)** - Azure Active Directory management
- **[Microsoft 365 Admin Center](https://admin.microsoft.com/)** - M365 administration
- **[PowerShell 7](https://github.com/PowerShell/PowerShell)** - Latest PowerShell version
- **[Visual Studio Code](https://code.visualstudio.com/)** - Recommended code editor with PowerShell extension

### Automation & DevOps

- **[AutoPilot](https://learn.microsoft.com/en-us/mem/autopilot/windows-autopilot)** - Modern device provisioning
- **[Azure DevOps](https://azure.microsoft.com/en-us/products/devops/)** - CI/CD for automation
- **[GitHub Actions](https://github.com/features/actions)** - Workflow automation
- **[Chocolatey](https://chocolatey.org/)** - Package manager for Windows

### Monitoring & Reporting

- **[Microsoft Endpoint Analytics](https://learn.microsoft.com/en-us/mem/analytics/overview)** - Device analytics and insights
- **[Azure Monitor](https://azure.microsoft.com/en-us/products/monitor/)** - Application and infrastructure monitoring
- **[Power BI](https://powerbi.microsoft.com/)** - Business intelligence and reporting
- **[Log Analytics](https://learn.microsoft.com/en-us/azure/azure-monitor/logs/log-analytics-overview)** - Query and analyze log data

---

## Contributing

We welcome contributions from the community! Here's how you can help:

### How to Contribute

1. **Fork this repository**
2. **Add your resource** to the appropriate section
3. **Submit a pull request** with a clear description of what you're adding
4. **Follow the formatting guidelines** below

### Formatting Guidelines

- Use descriptive link text (not "click here")
- Include a brief description of what the resource offers
- Maintain alphabetical order within sections where appropriate
- Check that links are working before submitting
- For new sections, follow the existing markdown structure

### Suggesting New Sections

If you think a new category would be valuable, open an issue to discuss it before creating a pull request.

### Quality Standards

Resources should:
- Be relevant to Endpoint Management (Intune, ConfigMgr, PowerShell, etc.)
- Provide educational, informational, or networking value
- Be from reputable sources
- Have accurate and working links

---

## License

This resource list is provided as-is for educational and community purposes. Links to external sites are the property of their respective owners.

---

## Acknowledgments

Thank you to the entire Microsoft Endpoint Management community for sharing knowledge, creating tools, and helping each other succeed!

**Last Updated:** February 2026

---

*This resource hub is maintained by the community. Star this repository to keep up with updates!*
